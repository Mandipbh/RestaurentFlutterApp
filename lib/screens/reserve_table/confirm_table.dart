...
class ConfirmedTable extends ConsumerWidget {
  ...
  void _addToCalendar(BuildContext context) {
    if (date == null || time == null || restaurantName == null) return;

    DateTime startDateTime = DateFormat('yyyy-MM-dd HH:mm')
        .parse("${DateFormat('yyyy-MM-dd').format(date!)} $time");

    final event = Event(
      title: "Table Reservation at $restaurantName",
      description:
          "Reservation for $numberOfPeople people at $restaurantName in $cityName.\nTables: $tableNos",
      location: restaurantName!,
      startDate: startDateTime,
      endDate: startDateTime.add(const Duration(hours: 2)),
    );
